...
fs.readFile('../result_ikyu/ikyu_paging_adult1.json', (err, data) => {
    if (err) throw err;
    ...
    hotelPage.forEach(async(prefItem) => {
        ...
        prefItem.list_page.forEach(async(pageItem) => {
                ...
                $('section[itemprop="itemListElement"]')
                    .map((_, hotel) => {
                        ...
                        return {                            
                            ...
                            plan_link: $hotel.find('.moreRooms').find('a').attr('href').replace('#room-plan-list', ''),
                        }
                }).toArray();
            const mergedArray = [];
            const crawl = async url => {
                ...
                fs.writeFileSync('../result_ikyu/result_ikyu_ranking_meal_type_adult1.json', data);
            };
