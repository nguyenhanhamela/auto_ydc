...
fs.readFile('../result_ikyu/ikyu_paging.json', (err, data) => {
    if (err) throw err;
    ...
    hotelPage.forEach(async(prefItem) => {
        ...
        prefItem.list_page.forEach(async(pageItem) => {
            if (pageItem <= 1) {
                var url = "https://www.ikyu.com/" + prefItem.medium_area_id + "/" + prefItem.small_area_id + "/"
            }
            else if (pageItem > 1) {
                var url = "https://www.ikyu.com/" + prefItem.medium_area_id + "/" + prefItem.small_area_id+ "/p" + pageItem + "/"
            }
                ...
                $('section[itemprop="itemListElement"]')
                    .map((_, hotel) => {
                        ...
                        return {                            
                            ...
                            adult_amount: $('.inline-block .mr-2').last().text(),
            ...
            const crawl = async url => {
                ...
                fs.writeFileSync('../result_ikyu/result_ikyu_ranking.json', data);
            };
