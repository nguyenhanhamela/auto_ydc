...
const jalanUrl = ["https://www.jalan.net/370000/LRG_370500/",
	"https://www.jalan.net/200000/LRG_200400/"
...
const jalanUrlFix = "?contHideFlg=1&roomCount=1&adultNum=2&distCd=01&child5Num=&mealType=&photo=1&child4Num=&roomCrack=200000&dateUndecided=1&child3Num=&afCd=&child1Num=&child2Num=&childPriceFlg=0,0,0,0,0&rootCd=041";
const arrHotels = []
// const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
const hotelName = []
const getTable = async () => {
	for (let i = 0; i < jalanUrl.length; i++) {
		let time = new Date();
		let option = new Chrome.Options().headless();
		let driver = await browser.forBrowser("chrome").build();//.setChromeOptions(option).build();
		// await (async function test() {
			try {
				await driver.get(jalanUrl[0] + jalanUrlFix).then(async () => {
					await driver.findElements(swd.By.className('p-searchResultItem__summaryInner')).then(async (hotels) => {
						await driver.sleep(60)
						hotels.forEach(async(hotel) => {
							// const getName = await hotel.findElement(swd.By.className('p-searchResultItem__facilityName')).getAttribute("innerHTML");
							hotelName.push(await hotel.findElement(swd.By.className('p-searchResultItem__facilityName')).getText().toUpperCase());
						})
						// hotels.map((data) => {
						// 	lstHotel.push({
						// 		hotelName: data.findElement(swd.By.className('p-searchResultItem__facilityName')).getText(),
						// 	})
						// })
